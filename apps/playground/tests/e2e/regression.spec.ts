...
const TABLE_SHOWCASE_ROUTE =
  "/scenes/table-showcase?lang=zh-CN&theme=dark&collab=0";
...
async function revealBlockHandleForSelector(
  page: Page,
  selector: string,
  timeoutMs = 5000,
) {
...
async function dragCurrentBlockHandleToTargetParagraph(
  ...
  return await page.evaluate(
    ({ id, targetPlacement }) => {
      ...
      const clientY =
        targetPlacement === "after" ? rect.bottom - 4 : rect.top + 6;
...
async function selectTextInTableCell(page: Page, cellText: string, steps = 4) {
  const cell = page
    .locator(".ProseMirror table td", { hasText: cellText })
    .first();
...
async function triggerTopToolbarCommand(
  page: Page,
  command: string,
): Promise<"direct" | "more"> {
  ...
  const commandBtn = page
    .locator(`.toolbar-more-menu button[data-command="${command}"]`)
    .first();
...
async function focusCodeBlockEndByText(page: Page, codeText: string) {
  const code = page
    .locator(".code-block-wrapper code", { hasText: codeText })
    .first();
...
async function getCodeBlockProbe(page: Page, pastedText: string) {
  return await page.evaluate((needle) => {
    ...
    const afterParagraph = document.querySelector(
      "#be-code-after-paragraph",
    ) as HTMLElement | null;
...
describe("regression e2e (H2)", () => {
  ...
  test("H2.18 粘贴落点：粘贴后仍留在当前段落连续编辑", async () => {
    ...
    const combinedBlockIndex = await getTopBlockIndexByText(
      page,
      `${pasted}${tail}`,
    );
  ...
  test("H2.20 评论链路一致性：顶部工具栏与选区工具栏均可预填且不自动保存", async () => {
    ...
    await selectionToolbar
      .locator('button[data-command="addComment"]')
      .first()
      .click();
  ...
  test("H3.1 评论线程：回复流程 + 解决/重开状态切换一致", async () => {
    ...
    const openFilter = page
      .locator(".comment-filter-btn", { hasText: /未解决|Open/i })
      .first();
    ...
    await expect
      .poll(
        async () => await reopenedThread.locator(".comment-action-btn--success").count(),
        { timeout: 3000 },
      )
      .toBe(1);
  ...
  test("H3.2 工具栏下拉：键盘导航与 Esc 关闭一致", async () => {
    ...
    await expect
      .poll(
        async () =>
          await menu.locator(".dropdown-item.keyboard-focus").count(),
        { timeout: 3000 },
      )
      .toBeGreaterThan(0);
    ...
    await expect
      .poll(async () => await menu.isVisible(), { timeout: 3000 })
      .toBe(false);
  ...
  test("H3.3 暗黑模式弹层：tooltip/dropdown 对比度可读", async () => {
    ...
    const underlineBtn = tooltipBar
      .locator('button[data-command="toggleUnderline"]')
      .first();
    ...
    const dropdownStyles = await dropdownMenu.evaluate((el) => {
      const style = getComputedStyle(el as HTMLElement);
      const firstItem = (el as HTMLElement).querySelector(
        ".dropdown-item",
      ) as HTMLElement | null;
  ...
  test("H3.5 链接编辑：更新地址/文本并支持撤销重做", async () => {
    ...
    await waitForVisible(dialog);
    await dialog
      .locator("input.be-input-control")
      .first()
      .fill("#be-code-after-paragraph");
  ...
  test("H3.6 块复制：duplicate block 生成结构一致副本", async () => {
    ...
    const countBefore = await page
      .locator(".ProseMirror p", { hasText: targetText })
      .count();
    ...
    await expect
      .poll(
        async () =>
          await page.locator(".ProseMirror p", { hasText: targetText }).count(),
        { timeout: 3000 },
      )
      .toBe(countBefore + 1);
  ...
  test("H3.7 粘贴图片链接：空选区自动转图片节点", async () => {
    ...
    await expect
      .poll(
        async () =>
          await page.locator(`.ProseMirror img[src="${imageUrl}"]`).count(),
        { timeout: 3000 },
      )
      .toBeGreaterThan(0);

    await expect(
      await page.locator(`.ProseMirror a[href="${imageUrl}"]`).count(),
    ).toBe(0);
  ...
  test("H3.8 块拖拽排序：手柄拖拽可重排且目标有高亮反馈", async () => {
    ...
    const sourceParagraph = page
      .locator(".ProseMirror p", { hasText: sourceText })
      .first();
  ...
  test("H3.9 图片节点：对齐切换与说明编辑保持稳定", async () => {
    ...
    const rightAligned = await page.evaluate(() => {
      const figure = document.querySelector(
        ".ProseMirror .be-image-figure",
      ) as HTMLElement | null;
      const btn = figure?.querySelector(
    ...
    const targetLanguage = await page.evaluate(() => {
      ...
      const item = Array.from(menu.querySelectorAll<HTMLElement>(".lang-item")).find(
        (node) =>
          ...
          ["typescript", "javascript", "python", "json"].includes(
  ...
  test("H3.11 表格 handle：可稳定点击并触发整表高亮", async () => {
    ...
    await expect
      .poll(
        async () =>
          ((await table.getAttribute("class")) || "").includes("be-table-selected"),
        { timeout: 3000 },
      )
      .toBe(true);
  ...
  test("H3.12 图片块拖拽：重排后不新增空段落", async () => {
    ...
    const sourceImage = page
      .locator('.ProseMirror img[alt="drag-image-source"]')
      .first();
    ...
    const targetBefore = await getTopBlockIndexBySelector(
      page,
    ...
    await revealBlockHandleForSelector(
      page,
      '.ProseMirror img[alt="drag-image-source"]',
    );
  ...
  test("H3.14 表格专项：表格与块 handle 边界可区分", async () => {
    ...
    await expect
      .poll(
        async () =>
          ((await mainTable.getAttribute("class")) || "").includes("be-table-selected"),
        { timeout: 3000 },
      )
      .toBe(true);
  ...
  test("H3.15 图片操作边界：图片操作区与块 handle 可区分", async () => {
    ...
    await expect.poll(async () => await isBlockHandleVisible(page), { timeout: 3000 }).toBe(
      false,
    );
  ...
  test("H3.16 代码块语言连续切换：连续 3 次不残留加载态", async () => {
    ...
    await expect
      .poll(
        async () =>
          ((await codeBlock
            .locator(".code-block-lang-select")
            .first()
            .getAttribute("class")) || ""),
        { timeout: 5000 },
      )
