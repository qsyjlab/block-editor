        ...
        for (const child of listItem.content) {
            if (child.type === 'paragraph') {
                ...
                paragraphs.push(new Paragraph({
                    ...
                    indent: type === 'task' ? { left: convertInchesToTwip(0.25 * (level + 1)) } : undefined
                }))
        ...
        for (const row of node.content) {
            ...
            if (row.content) {
                for (const cell of row.content) {
                    ...
                    const validChildren = cellChildren.filter(c => c instanceof Paragraph || c instanceof Table)
      ...
      if (node.type === 'text') {
        ...
        if (linkMark) {
            return new ExternalHyperlink({
                children: [new TextRun({ ...options, style: 'Hyperlink', color: '0000FF', underline: { type: UnderlineType.SINGLE } })],
