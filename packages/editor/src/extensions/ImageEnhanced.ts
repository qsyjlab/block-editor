...
const ENABLE_IMAGE_CAPTION = false;
  ...
  addAttributes() {
    return {
      ...
      align: {
        ...
        parseHTML: (el) =>
          (el.closest("figure")?.getAttribute("data-align") as ImageAlign) ||
          "center",
  ...
  addNodeView() {
    return ({ node, getPos, editor }) => {
      ...
      if (ENABLE_IMAGE_CAPTION) {
        ...
        nextFigcaption.setAttribute(
          "data-placeholder",
          this.options.i18n.captionPlaceholder,
        );
      ...
      return {
        ...
        ignoreMutation: (mutation) => {
          ...
          if (figcaption && figcaption.contains(mutation.target as Node))
            return true;
          return false;
        },
        update(updatedNode) {
