...
function resolveSelectionToolbarRegion(editor: Editor): EditorUIRegion {
  ...
  const allowedRegions: EditorUIRegion[] = ['toolbar', 'editor', 'outline', 'comment', 'overlay']
  return allowedRegions.includes(region as EditorUIRegion) ? (region as EditorUIRegion) : 'overlay'
}
...
function resolveSelectionToolbarAppendTarget(editor: Editor): HTMLElement {
  ...
  if (uiRoot) {
    const regionHost = uiRoot.querySelector(`[data-be-region="${region}"]`) as HTMLElement | null
