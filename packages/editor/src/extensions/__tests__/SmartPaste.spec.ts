...
describe('SmartPaste', () => {
  ...
  it('should sanitize dangerous HTML and insert cleaned content', () => {
    class FakeDOMParser {
      parseFromString(html: string) {
        ...
        const parseParagraph = () => {
          ...
          rawAttrs.replace(/(\w[\w-]*)="([^"]*)"/g, (_full, key: string, value: string) => {
            attrs[key] = value
            return ''
          })
          return { attrs, text }
        ...
        const writeParagraph = (attrs: Record<string, string>, text: string) => {
          const attrText = Object.entries(attrs)
            ...
            .join('')
          state.html = state.html.replace(/<p[^>]*>[\s\S]*?<\/p>/i, `<p${attrText}>${text}</p>`)
        }
        ...
        const querySelectorAll = (selector: string) => {
          if (selector === 'script') {
            ...
            return [
              {
                remove: () => {
                  state.html = state.html.replace(/<script[\s\S]*?<\/script>/gi, '')
                },
              },
            ]
          }
          ...
          if (selector === '[style]') {
            ...
            return [
              {
                getAttribute: (name: string) => (name === 'style' ? paragraph.attrs.style : null),
    ...
    expect(inserted).toMatch(/<p style="font-size:\s*16px;\s*color:\s*red">hello<\/p>/)
