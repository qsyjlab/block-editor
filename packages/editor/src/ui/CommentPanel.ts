...
function splitLegacyQuotedComment(text: string): {
  ...
  const match =
    text.match(/^关于「(.+?)」：\s*(.*)$/s) || text.match(/^About ["“](.+?)["”]:\s*(.*)$/is)
...
const DEFAULT_COMMENT_PANEL_I18N: CommentPanelI18n = resolveEditorI18n('en-US').commentPanel
...
export class CommentPanel {
  ...
  constructor(editorCore: EditorCore, container: HTMLElement, i18n?: Partial<CommentPanelI18n>) {
    ...
    title.style.cssText = 'margin:0;font-size:14px;font-weight:600;color:var(--text-color);'
  ...
  private handleOpenCommentPanel() {
    ...
    const current = !selection.empty
      ? buildSelectionSnapshot(
          { from: selection.from, to: selection.to },
          this.editorCore.editor.state.doc.textBetween(selection.from, selection.to, ' '),
        )
      : null
    if (current) {
  ...
  public focusThread(commentId: string) {
    ...
    const item = this.listEl.querySelector(`[data-comment-id="${commentId}"]`) as HTMLElement | null
    if (!item) return
  ...
  private renderSelectionHint() {
    const empty = this.editorCore.editor.state.selection.empty && !this.pendingSelection
    this.hintEl.textContent = empty ? this.i18n.selectionHintEmpty : this.i18n.selectionHintReady
  ...
  private render() {
    ...
    if (threads.length === 0) {
      ...
      empty.textContent =
        this.filter === 'resolved' ? this.i18n.emptyResolved : this.i18n.emptyNoComments
  ...
  private renderThread(thread: CommentThread): HTMLElement {
    ...
    if (!thread.resolved) {
      const resolveBtn = this.createActionBtn('✓', this.i18n.resolveAction, 'success')
      ...
      const reopenBtn = this.createActionBtn('↺', this.i18n.reopenAction, 'primary')
    ...
    const deleteBtn = this.createActionBtn('✕', this.i18n.deleteAction, 'danger')
    ...
    const { quoteText: legacyQuote, bodyText } = splitLegacyQuotedComment(thread.text)
  ...
  destroy() {
    this.unsubscribe()
    this.editorCore.events.off('openCommentPanel', this.openCommentPanelHandler)
