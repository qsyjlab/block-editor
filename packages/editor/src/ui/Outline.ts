...
export class Outline {
  ...
  private render() {
    ...
    if (this.headings.length === 0) {
      ...
      this.headings.forEach((h) => {
        ...
        li.style.fontSize =
          h.level === 1 ? "15px" : h.level === 2 ? "13px" : "12px";
  ...
  private getNearestBlockContext(
    pos: number,
  ): { blockId: string | null; pos: number } | null {
  ...
  private navigateToPos(pos: number) {
    const safePos = Math.max(
      1,
      Math.min(pos + 1, this.editorCore.editor.state.doc.content.size),
    );
  ...
  private getActiveHeadingByViewport(): number | null {
    ...
    const anchorY =
      viewportTop +
      Math.max(
        OUTLINE_ACTIVE_TOP_PADDING,
        Math.floor(viewportHeight * OUTLINE_ACTIVE_RATIO),
      );
    ...
    if (
      scrollHeight > 0 &&
      scrollTop + clientHeight >= scrollHeight - OUTLINE_BOTTOM_LOCK_GAP
    ) {
