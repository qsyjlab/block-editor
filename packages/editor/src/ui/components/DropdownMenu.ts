...
export function createDropdownMenu(options: DropdownMenuOptions = {}): HTMLElement {
  ...
  menu.className = ['toolbar-dropdown-menu', options.className || ''].filter(Boolean).join(' ')
...
export function getFocusableDropdownItems(menu: HTMLElement): HTMLElement[] {
  return Array.from(menu.querySelectorAll<HTMLElement>('.dropdown-item:not([disabled])'))
}
