...
class ImagePreviewModal {
  ...
  private renderCurrent() {
    ...
    this.hintEl.textContent =
      current.alt || current.title || this.i18n.previewImage;
  ...
  private zoomIn() {
    this.scale = Math.min(
      MAX_ZOOM,
      Number((this.scale + ZOOM_STEP).toFixed(2)),
    );
    this.applyTransform();
  }
  ...
  private collectItems(sourceImage: HTMLImageElement) {
    ...
    const nodes = Array.from(
      root.querySelectorAll<HTMLImageElement>(
        'img[data-be-image-preview="true"]',
      ),
    );
...
export function openImagePreviewFromImage(
