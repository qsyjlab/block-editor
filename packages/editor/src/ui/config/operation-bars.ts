...
import type { DropdownOptionConfig, ToolbarItemType } from '../toolbar/ToolbarRegistry'
...
function patchGroups(groups: ToolbarItemType[][], ctx: PatchContext): ToolbarItemType[][] {
...
function reorderGroup(group: ToolbarItemType[], itemOrder: string[]): ToolbarItemType[] {
...
function getItemOrderRank(item: ToolbarItemType, orderMap: Map<string, number>): number {
...
function getOverride(keys: string[], ctx: PatchContext): ToolbarItemOverride | null {
  for (const key of keys) {
...
function shouldHideItem(
  ...
  if (
    (item.type === 'button' || item.type === 'color') &&
    item.command &&
    hiddenCommands.has(item.command)
  ) {
    return true
  }
...
function patchItem(
  ...
  if (next.type === 'dropdown') {
    ...
    const itemLabelOverride = ctx.labelOverrides[next.label] || getOverrideI18nLabel(keys, ctx)
