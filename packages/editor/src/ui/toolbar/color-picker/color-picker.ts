...
export class ColorPicker {
  ...
  private render() {
    ...
    STANDARD_COLORS.forEach((color) => {
      ...
      check.style.color = isLight ? 'var(--text-color)' : 'var(--brand-solid-text)'
  ...
  private open() {
    ...
    // Update active state
    const currentColor = this.editorCore.editor.getAttributes('textStyle').color
    ...
    if (currentColor) {
      ...
      const activeItem = Array.from(allItems).find(
        (el) => (el as HTMLElement).dataset.value?.toLowerCase() === currentColor.toLowerCase(),
      )
      if (activeItem) {
  ...
  private setColor(color: string, close = true) {
    ...
    // Update preview
    const preview = this.trigger.querySelector('.color-preview-box') as HTMLElement
    if (preview) preview.style.backgroundColor = color

    // Update text input
    const textInput = this.dropdown.querySelector('.custom-color-text-input') as HTMLInputElement
    if (textInput && textInput.value.toUpperCase() !== color.toUpperCase()) {
      textInput.value = color
    }

    // Update color input preview
    const colorWrapper = this.dropdown.querySelector('.custom-color-wrapper') as HTMLElement
